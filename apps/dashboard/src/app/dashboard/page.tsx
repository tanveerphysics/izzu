...
import { ArrowUpRight, Plus, Users, Globe, Activity, Copy, Check, ChevronDown, Key, Eye, EyeOff, MapPin, Camera } from "lucide-react";
...
export default function DashboardOverview() {
    ...
    const [createdKeys, setCreatedKeys] = useState<{ apiKey: string; secretKey: string } | null>(null);
    const [stats, setStats] = useState<Stats>({ totalUsers: 0, activeFaceIds: 0, locationsTracked: 0, totalProjects: 0 });
    ...
    const handleCreateProject = async () => {
        ...
        if (res.ok) {
            setCreatedKeys({ apiKey: data.project.apiKey, secretKey: data.project.secretKey });
        ...
        <div className="space-y-8">
            ...
            {selectedProject && (
                ...
                >
                    <div className="flex items-start justify-between mb-6">
                        <div>
                            <h2 className="text-xl font-bold text-white mb-1">🔐 Integrate Face ID into Your App</h2>
                            <p className="text-zinc-400 text-sm">Copy this code to add mandatory Face ID + Location tracking to your project.</p>
                        </div>
                    ...
                    <div className="mb-4">
                        ...
                        <div className="bg-black rounded-lg border border-zinc-800 p-4 font-mono text-sm text-zinc-300 flex items-center justify-between">
                            ...
                            <button onClick={() => copyToClipboard(sdkInstallCode, "install")} className="text-zinc-500 hover:text-white">
                    ...
                    {/* Usage Code */}
                    <div className="mb-4">
                        <div className="text-xs text-zinc-500 uppercase font-medium mb-2">2. Implement Face Auth</div>
                        <div className="bg-black rounded-lg border border-zinc-800 p-4 font-mono text-sm text-zinc-300 overflow-x-auto relative">
                            ...
                            >
                                {copied === "usage" ? <Check className="w-4 h-4 text-emerald-500" /> : <Copy className="w-4 h-4" />}
                            </button>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="bg-zinc-950 rounded-lg border border-zinc-800 p-3 font-mono text-xs text-zinc-300 flex items-center justify-between">
                                ...
                                <Copy className="w-3 h-3 text-zinc-600 cursor-pointer hover:text-white" onClick={() => copyToClipboard(selectedProject.id, "pid")} />
                            </div>
                        </div>
                        <div>
                            <div className="text-xs text-zinc-500 uppercase font-medium mb-2">API Key (Publishable)</div>
                            <div className="bg-zinc-950 rounded-lg border border-zinc-800 p-3 font-mono text-xs text-zinc-300 flex items-center justify-between">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    ...
                    { label: "Active Face IDs", value: stats.activeFaceIds, icon: Camera },
                    { label: "Locations Tracked", value: stats.locationsTracked, icon: MapPin },
                ].map((stat, i) => (
                    <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        ...
                        {!createdKeys ? (
                            <>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-xs text-zinc-500 uppercase font-medium mb-2">Project Name</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-xs text-zinc-500 uppercase font-medium mb-2">Project Slug</label>
                                        <input
                                            type="text"
                                            value={newProjectSlug}
                                            onChange={e => setNewProjectSlug(e.target.value.toLowerCase().replace(/\s/g, "-"))}
                                            className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-3 text-white"
                                            placeholder="my-secure-app"
                                        />
                                    </div>
                                ...
                                <div className="flex gap-3 mt-6">
                                    <button onClick={() => setShowCreateModal(false)} className="flex-1 py-3 bg-zinc-800 rounded-lg hover:bg-zinc-700">Cancel</button>
                                    <button onClick={handleCreateProject} className="flex-1 py-3 bg-blue-600 rounded-lg hover:bg-blue-500 font-semibold">Create</button>
                                </div>
                            </>
                        ) : (
                            <>
                                <div className="text-center mb-6">
                                    ...
                                    <p className="text-zinc-400 text-sm mt-2">Save these keys! The secret key won't be shown again.</p>
                                </div>
                                <div className="space-y-4">
                                    <div>
                                        <label className="text-xs text-zinc-500 uppercase font-medium">API Key</label>
                                        <div className="mt-1 bg-zinc-950 border border-zinc-800 rounded-lg p-3 font-mono text-xs text-zinc-300 break-all">{createdKeys.apiKey}</div>
                                    </div>
                                    <div>
                                        <label className="text-xs text-zinc-500 uppercase font-medium">Secret Key (Save Now!)</label>
                                        <div className="mt-1 bg-red-950/50 border border-red-500/30 rounded-lg p-3 font-mono text-xs text-red-300 break-all">{createdKeys.secretKey}</div>
                                    </div>
                                </div>
                                <button
                                    onClick={() => { setShowCreateModal(false); setCreatedKeys(null); }}

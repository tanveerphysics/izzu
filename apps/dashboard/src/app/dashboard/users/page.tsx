...
import { Search, Filter, Shield, Mail, Calendar, Ban, Trash2 } from "lucide-react";
            ...
            <div className="rounded-xl border border-zinc-800/50 bg-zinc-900/20 overflow-hidden">
                            ...
                            {loading ? (
                                Array.from({ length: 5 }).map((_, i) => (
                                    <tr key={i} className="animate-pulse">
                                        <td className="px-6 py-4"><div className="h-10 w-40 bg-zinc-800 rounded" /></td>
                                        ...
                                        <td className="px-6 py-4"><div className="h-8 w-8 bg-zinc-800 rounded" /></td>
                                    </tr>
                                ))
                                ...
                                filteredUsers.map((user) => (
                                    ...
                                    >
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="relative group/avatar cursor-pointer">
                                                    <img src={user.avatar} alt="Login Face" className="w-12 h-12 rounded-xl bg-zinc-800 object-cover border-2 border-zinc-800 group-hover/avatar:border-blue-500 transition-colors" />
                                                    <div className="absolute -bottom-1 -right-1 bg-zinc-950 rounded-full p-0.5">
                                                        {user.provider === "google" && <span className="w-4 h-4 block bg-red-500 rounded-full border-2 border-zinc-950" />}
                                                        {user.provider === "github" && <span className="w-4 h-4 block bg-white rounded-full border-2 border-zinc-950" />}
                                                        {user.provider === "email" && <Mail className="w-4 h-4 text-zinc-400 bg-zinc-950" />}
                                                    </div>
                                                </div>
                                                <div>
                                                    <div className="font-medium text-zinc-200">{user.email}</div>
                                                    {user.mobile && <div className="text-xs text-zinc-500 font-mono mt-0.5">{user.mobile}</div>}
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${user.status === "active"
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button className="p-2 hover:bg-zinc-800 rounded-lg text-zinc-400 hover:text-white" title="View Details">
                                                    <Shield className="w-4 h-4" />
                                                </button>
                                                <button className="p-2 hover:bg-zinc-800 rounded-lg text-zinc-400 hover:text-red-400" title="Ban User">
                                                    <Ban className="w-4 h-4" />
                                                </button>
                                                <button className="p-2 hover:bg-zinc-800 rounded-lg text-zinc-400 hover:text-red-500" title="Delete User">
                ...
                <div className="px-6 py-4 border-t border-zinc-800/50 text-xs text-zinc-500 flex justify-between items-center bg-zinc-900/30">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button className="px-3 py-1 bg-zinc-800 rounded hover:bg-zinc-700 disabled:opacity-50" disabled>Next</button>
                    </div>
                </div>
            </div>
        </div>

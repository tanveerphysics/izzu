...
import { Webhook, Plus, Copy, Trash2, Check, AlertTriangle, RefreshCw, Zap } from "lucide-react";
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-zinc-400 text-sm">Receive real-time event notifications via HTTP callbacks.</p>
                </div>
            ...
            <div className="flex items-start gap-3 p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                ...
                <div>
                    ...
                    <p className="text-xs text-blue-400/80 mt-1">
                        Webhooks notify your server when events occur. Use them to sync data, send notifications, or trigger workflows.
                    </p>
                </div>
            </div>
                            ...
                            <div className="flex items-start justify-between">
                                <div className="flex-1">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-2 h-2 rounded-full ${webhook.isActive ? "bg-emerald-400" : "bg-zinc-600"}`} />
                                ...
                                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    {webhook.secret && (
                                        <button
                                            ...
                                            title="Copy Secret"
                                        >
                                            {copiedId === webhook.id ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
                                        </button>
                                    )}
                                    <button className="p-2 hover:bg-zinc-800 rounded-lg text-zinc-400 hover:text-red-400" title="Delete">

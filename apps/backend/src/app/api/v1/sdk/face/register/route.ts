...
import { db, projects, endUsers, eq, and } from "@izzu/db";
...
export async function POST(req: NextRequest) {
    try {
        ...
        const faceRes = await fetch("http://localhost:8000/register", {
            method: "POST",
            body: faceFormData,
        });
        ...
        if (!faceRes.ok || !faceData.success) {
            return NextResponse.json({ error: faceData.error || "Face registration failed" }, { status: 400 });
        }

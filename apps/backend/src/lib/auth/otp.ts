...
export class OtpService {
     ...
     */
    async sendOtp(identifier: string, channel: "email" | "sms" = "email"): Promise<{ success: boolean; otp?: string }> {
        const cleanIdentifier = channel === "email"
        ...
        if (!twilioClient || !TWILIO_VERIFY_SID) {
            throw new Error("Twilio not configured. Set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_VERIFY_SID");
        }
        ...
        try {
            ...
            console.log(`[TWILIO] OTP sent to ${cleanIdentifier} via ${channel} (Status: ${verification.status})`);
            return { success: true };
        } catch (error: any) {

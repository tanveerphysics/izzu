import { db, identities, endUsers, eq } from "@izzu/db";

// Mock configuration - in real app, these come from Tenant/Project settings in DB
const PROVIDERS: Record<string, { clientId: string; clientSecret: string; authUrl: string; tokenUrl: string; scope: string }> = {
    google: {
...
export class OAuthService {
     ...
     */
    async handleCallback(provider: string, code: string, projectId: string) {
        ...
        let identity = await db.query.identities.findFirst({
            where: (t, { and, eq }) => and(eq(t.provider, provider as any), eq(t.providerId, userInfo.id))
        });

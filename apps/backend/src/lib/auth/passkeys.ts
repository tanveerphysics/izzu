...
import { db, passkeys, endUsers, type NewPasskey, eq } from "@izzu/db";
...
export class PasskeyService {
    ...
    async generateRegisterOptions(userId: string, userEmail: string) {
        ...
        const userPasskeys = await db
            .select()
            .from(passkeys)
            .where(eq(passkeys.endUserId, userId));
    ...
    async verifyRegister(userId: string, body: any, expectedChallenge: string) {
        ...
        if (verification.verified && verification.registrationInfo) {
            ...
            const credentialDeviceType = info.credentialDeviceType || info.credential?.transports;
